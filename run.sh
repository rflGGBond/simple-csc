...

{
    ...
    echo "prompted_model:          ${prompted_model:=../autodl-tmp/Qwen2.5-7B-Base}"
    echo "config_path:             ${config_path:=configs/c2ec_config.yaml}"
    ...
    for dataset in "${datasets[@]}"; do
        ...
        mkdir -p "results/Qwen2.5-7B-Base/Qwen2.5-7B-Base-v1/${dataset_name}"
        python -u run.py \
            --input-file "datasets/${dataset}"  \
            --path "results/Qwen2.5-7B-Base/Qwen2.5-7B-Base-v1/${dataset_name}"  \
            --model-name "${model}"  \
            ...
            --distortion-model-smoothing "${prob_smooth}" | tee "results/Qwen2.5-7B-Base/Qwen2.5-7B-Base-v1/${dataset_name}/prediction.log"
        if [[ "$dataset_name" =~ "ecspell" || "$dataset_name" =~ "sighan_rev" || "$dataset_name" =~ "lemon_v2" || "$dataset_name" =~ "cscd_ime" || "$dataset_name" =~ "mcsc" ]]; then
            ...
            python eval/evaluate.py \
                ...
                --hypo "results/Qwen2.5-7B-Base/Qwen2.5-7B-Base-v1/${dataset_name}/prediction.txt"  \
                --to_halfwidth  \
