...
from final_lmcsc.generation import (
    process_reward_beam_search,
    token_transformation_to_probs,
    get_distortion_probs,
    distortion_guided_beam_search,
)
from final_lmcsc.model import AutoLMModel, LMModel
from final_lmcsc.obversation_generator import NextObversationGenerator
from final_lmcsc.streamer import BeamStreamer
from final_lmcsc.transformation_type import TransformationType
from final_lmcsc.common import MIN, OOV_CHAR
from final_lmcsc.utils import Alignment, clean_sentences, rebuild_sentences
...
class LMCorrector:
    ...
    def __init__(
        ...
        # ===== 新增：键盘相似度参数 =====
        use_keyboard_similarity: bool = None,
        keyboard_tau: float = None,
        keyboard_w_min: float = None,
        keyboard_w_max: float = None,
        # ================================
        *args,
        **kwargs,
    ):
        ...
        # ===== 新增：读取键盘相似度配置 =====
        keyboard_config = self.config.get('keyboard_similarity', {})
        self.use_keyboard_similarity = use_keyboard_similarity if use_keyboard_similarity is not None else keyboard_config.get('enabled', False)
        self.keyboard_tau = keyboard_tau if keyboard_tau is not None else keyboard_config.get('tau', 3.0)
        self.keyboard_w_min = keyboard_w_min if keyboard_w_min is not None else keyboard_config.get('w_min', 3.75)
        self.keyboard_w_max = keyboard_w_max if keyboard_w_max is not None else keyboard_config.get('w_max', 8.91)
        # =====================================
    ...
    def decorate_model_instance(self) -> None:
        ...
        # ===== 新增：设置键盘相似度参数到模型 =====
        self.model.use_keyboard_similarity = self.use_keyboard_similarity
        self.model.keyboard_tau = self.keyboard_tau
        self.model.keyboard_w_min = self.keyboard_w_min
        self.model.keyboard_w_max = self.keyboard_w_max
        # ==========================================
    ...
    def print_params(self):
        ...
        # ===== 新增：打印键盘相似度参数 =====
        print(f"Use keyboard similarity: {self.use_keyboard_similarity}")
        if self.use_keyboard_similarity:
            print(f"  - tau: {self.keyboard_tau}")
            print(f"  - w_min: {self.keyboard_w_min}")
            print(f"  - w_max: {self.keyboard_w_max}")
        # =====================================
