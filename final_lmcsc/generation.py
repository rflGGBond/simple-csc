...
from final_lmcsc.obversation_generator import BaseObversationGenerator

# ===== 新增：键盘拼音相似度导入 =====
from final_lmcsc.keyboard_weight import get_keyboard_weight
# ====================================
...
def token_transformation_to_probs(self, observed_sequence: str) -> Tuple[List[int], List[float], dict]:
    ...
    支持键盘拼音相似度：
    - 当 self.use_keyboard_similarity = True 时，对拼音相关的替换使用连续的键盘距离权重
    - 否则使用原有的离散类型权重
    ...
    # ===== 新增：获取键盘相似度配置 =====
    use_keyboard = getattr(self, 'use_keyboard_similarity', False)
    kbd_tau = getattr(self, 'keyboard_tau', 3.0)
    kbd_w_min = getattr(self, 'keyboard_w_min', 3.75)
    kbd_w_max = getattr(self, 'keyboard_w_max', 8.91)
    # ====================================

    # 定义需要使用键盘相似度的拼音相关类型
    PINYIN_RELATED_TYPES = {'SAP', 'SIP', 'OTP'}
    # 定义保持原有权重的类型
    KEEP_ORIGINAL_TYPES = {'IDT', 'PTC', 'MIS', 'RED', 'ROR', 'IMP', 'SIS', 'OTS', 'UNR'}

    if use_keyboard:
        # ===== 新逻辑：使用键盘拼音相似度 =====
        def get_weight(trans, token_idx):
            """基于键盘拼音距离的连续权重（仅对拼音相关类型）"""
            cache_key = (trans, token_idx)
            if cache_key in cache:
                return cache[cache_key]
            
            token = self.transformation_type.idx_to_token.get(token_idx, '')
            
            w = 0.0
            for i, t in enumerate(trans):
                if t in PINYIN_RELATED_TYPES:
                    # 拼音相关类型：使用键盘拼音距离
                    if i < len(token) and i < len(observed_sequence):
                        orig_char = observed_sequence[i]
                        cand_char = token[i]
                        kbd_weight = get_keyboard_weight(
                            orig_char, cand_char, 
                            w_min=kbd_w_min, 
                            w_max=kbd_w_max, 
                            tau=kbd_tau
                        ...
                        # 索引越界，使用默认的 OTP 权重
                        w += self.distortion_probs.get('OTP', -8.91)
                else:
                    # 非拼音相关类型：使用原有权重
                    w += self.distortion_probs.get(t, -8.91)
            
            cache[cache_key] = w
            return w
        
        weight = [get_weight(trans, idx) for idx, trans in zip(indices, token_transformation.values())]
        # =========================================
...
#     _distortion_probs: torch.Tensor) -> torch.Tensor:
#     """
#     Non-JIT version of distortion_probs_to_cuda_jit for use in reranker.py
#     """
#     if template_tensor.dtype == torch.float16:
#         MIN_VAL = -1e4
#     else:
#         MIN_VAL = -1e32
#     distortion_probs = template_tensor.masked_fill(force_eos[:, None], MIN_VAL).view(batch_size, num_beams, vocab_size)
#     distortion_probs[_batch_indices, _beam_indices, _token_indices] = _distortion_probs
#     return distortion_probs.view(batch_beam_size, vocab_size)
# # ================================================
