...
from crf_lmcsc.obversation_generator import BaseObversationGenerator
...
def process_reward_beam_search(
    ...
    **model_kwargs,
    ...
    while True:
        ...
        # ========== 改进 1.2: 处理 distortion_probs ==========
        # 获取配置参数 (可通过环境变量设置)
        # DISTORTION_MODE: "clamp" (默认) 或 "normalize"
        # DISTORTION_CLAMP_MIN: clamp 模式下的最小值，默认 -1.0
        distortion_mode = os.getenv("DISTORTION_MODE", "clamp")
        distortion_clamp_min = float(os.getenv("DISTORTION_CLAMP_MIN", "-1.0"))
        
        if distortion_mode == "normalize":
            # 方式b: 逐步归一化，保证最大为0
            distortion_probs = distortion_probs - distortion_probs.max(dim=-1, keepdim=True).values
        else:
            # 方式a: clamp，若它本质是惩罚项
            distortion_probs = torch.clamp(distortion_probs, min=distortion_clamp_min, max=0.0)
        # ========== END 改进 1.2 ==========
        ...
        if self.use_faithfulness_reward:
            ...
            # ========== 改进 1.3: 限幅 faithfulness_coefficient ==========
            # FAITHFULNESS_CLAMP_MIN: 默认 1.0
            # FAITHFULNESS_CLAMP_MAX: 默认 1.5
            faithfulness_clamp_min = float(os.getenv("FAITHFULNESS_CLAMP_MIN", "1.0"))
            faithfulness_clamp_max = float(os.getenv("FAITHFULNESS_CLAMP_MAX", "1.5"))
            faithfulness_coefficient = torch.clamp(
                faithfulness_coefficient, 
                min=faithfulness_clamp_min, 
                max=faithfulness_clamp_max
            )
            # ========== END 改进 1.3 ==========
        ...
        # ========== 改进 1.1: 使用显式权重 λ 融合 prompt_LLM 分数 ==========
        # 原公式: next_token_scores = prompted_next_token_scores + reward
        # 改进: next_token_scores = reward + λ * prompted_next_token_scores
        # LAMBDA_WEIGHT: 默认 0.3，推荐值: 0.1, 0.3, 0.5, 1.0
        lambda_weight = float(os.getenv("LAMBDA_WEIGHT", "0.3"))
        next_token_scores = lambda_weight * reward + (1 - lambda_weight) * prompted_next_token_scores
        # ========== END 改进 1.1 ==========
