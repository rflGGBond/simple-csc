...
from cp_lmcsc.generation import (
    process_reward_beam_search,
    token_transformation_to_probs,
    get_distortion_probs,
    distortion_guided_beam_search,
)
from cp_lmcsc.model import AutoLMModel, LMModel
from cp_lmcsc.obversation_generator import NextObversationGenerator
from cp_lmcsc.streamer import BeamStreamer
from cp_lmcsc.transformation_type import TransformationType
from cp_lmcsc.common import MIN, OOV_CHAR
from cp_lmcsc.utils import Alignment, clean_sentences, rebuild_sentences
...
class LMCorrector:
    ...
    def __init__(
        ...
        confidence_penalty_lambda: float = None,
        confidence_score_reduction: str = None,
        confidence_batch_size: int = None,
        *args,
        **kwargs,
    ):
        """
        ...
        # Confidence-based penalty (optional): penalize non-equal-length edits when the original sentence is already fluent.
        conf_cfg = self.config.get("confidence_penalty", {}) if isinstance(self.config, dict) else {}
        self.confidence_threshold = confidence_threshold if confidence_threshold is not None else conf_cfg.get("threshold", None)
        self.confidence_penalty_lambda = confidence_penalty_lambda if confidence_penalty_lambda is not None else conf_cfg.get("lambda", 0.0)
        self.confidence_score_reduction = confidence_score_reduction if confidence_score_reduction is not None else conf_cfg.get("reduction", "mean")
        self.confidence_batch_size = confidence_batch_size if confidence_batch_size is not None else conf_cfg.get("batch_size", 8)
    ...
    def decorate_model_instance(self) -> None:
        ...
        # Confidence penalty hyper-params (optional)
        self.model.confidence_threshold = self.confidence_threshold
        self.model.confidence_penalty_lambda = self.confidence_penalty_lambda
        self.model.confidence_score_reduction = self.confidence_score_reduction
    ...
    def print_params(self):
        ...
        print(f"Confidence penalty lambda: {self.confidence_penalty_lambda}")
        print(f"Confidence score reduction: {self.confidence_score_reduction}")
    ...
    def _compute_src_confidence_scores(
        self,
        sentences: List[str],
        batch_size: int = 8,
        reduction: str = "mean",
    ) -> torch.Tensor:
        """Compute a fluency/confidence score for each source sentence using the *pure* LM.

        We approximate log p_LM(x) by summing token log-probabilities of the sentence itself.
        If `reduction == "mean"`, we return the average log-probability per (non-pad) token
        ...
        scores = []
        for start in range(0, len(sentences), max(1, int(batch_size))):
            batch = sentences[start : start + max(1, int(batch_size))]
            # Truncate to avoid extremely long sequences blowing up memory.
            enc = tokenizer(
                batch,
                return_tensors="pt",
                padding=True,
            ...
            # teacher-forcing next-token log probs
            target_ids = input_ids[:, 1:]
            token_lp = log_probs[:, :-1, :].gather(-1, target_ids.unsqueeze(-1)).squeeze(-1)
            ...
            if reduction == "sum":
                ...
                # default: mean log-prob per token
                score = sum_lp / token_counts
    ...
    def __call__(
        ...
        # Optional: compute source-sentence fluency/confidence for confidence-based length-change penalty
        src_confidence_scores = None
        if (
            self.confidence_penalty_lambda is not None
            and float(self.confidence_penalty_lambda) != 0.0
            and self.confidence_threshold is not None
        ):
            src_confidence_scores = self._compute_src_confidence_scores(
                processed_src,
                batch_size=self.confidence_batch_size,
                reduction=self.confidence_score_reduction,
            )
        ...
        if stream:
            ...
            generation_kwargs = dict(
                ...
                src_confidence_scores=src_confidence_scores,
