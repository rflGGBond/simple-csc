...
from cp_lmcsc.obversation_generator import BaseObversationGenerator
...
def distortion_guided_beam_search(
    ...
    **model_kwargs,
    ...
    # --- Confidence-based length-change penalty (optional) ---
    confidence_penalty_lambda = getattr(self, "confidence_penalty_lambda", 0.0) or 0.0
    confidence_threshold = getattr(self, "confidence_threshold", None)

    apply_confidence_penalty = (
        src_confidence_scores is not None
        and confidence_threshold is not None
        and float(confidence_penalty_lambda) != 0.0
    )

    if apply_confidence_penalty:
        if not torch.is_tensor(src_confidence_scores):
            src_confidence_scores = torch.tensor(
                src_confidence_scores, dtype=torch.float32, device=input_ids.device
            )
        else:
            src_confidence_scores = src_confidence_scores.to(input_ids.device)

        # Expand per-example confidence scores to per-beam
        src_confidence_scores = src_confidence_scores.repeat_interleave(num_beams)
        src_conf_high = src_confidence_scores > float(confidence_threshold)

        # Source lengths in characters (expanded to per-beam)
        src_lens = torch.tensor(
            getattr(observed_sequence_generator, "src_char_lens", [0] * batch_size),
            dtype=torch.float32,
            device=input_ids.device,
        ).repeat_interleave(num_beams)

        # Track predicted output length (character units) per beam
        pred_lens = torch.zeros((batch_beam_size,), dtype=torch.float32, device=input_ids.device)
    else:
        src_conf_high = None
        src_lens = None
        pred_lens = None
    ...
    while True:
        ...
        # Apply confidence-based penalty on *ending* with EOS when length differs from the source.
        if apply_confidence_penalty and pred_lens is not None and src_lens is not None and src_conf_high is not None:
            # If output length differs by >= 1 char, treat as length-change
            length_mismatch = (pred_lens - src_lens).abs() > 0.5
            apply_mask = (src_conf_high & length_mismatch).view(batch_size, num_beams)

            if apply_mask.any():
                beam_ids = torch.arange(num_beams, device=next_token_scores.device)
                for _eos_id in eos_token_id:
                    eos_positions = beam_ids * vocab_size + int(_eos_id)
                    eos_scores = next_token_scores[:, eos_positions]
                    penalty = float(confidence_penalty_lambda) * apply_mask.to(eos_scores.dtype)
                    next_token_scores[:, eos_positions] = eos_scores - penalty
        ...
        # Update predicted length tracking after beam re-ordering
        if apply_confidence_penalty and pred_lens is not None:
            _token_len = self.token_length.to(input_ids.device)
            chosen_len = _token_len[beam_next_tokens].to(pred_lens.dtype)

            # EOS / PAD do not contribute to output length
            eos_mask = torch.zeros_like(beam_next_tokens, dtype=torch.bool)
            for _eos_id in eos_token_id:
                eos_mask |= beam_next_tokens == int(_eos_id)
            chosen_len = torch.where(eos_mask, torch.zeros_like(chosen_len), chosen_len)

            if pad_token_id is not None:
                chosen_len = torch.where(
                    beam_next_tokens == pad_token_id, torch.zeros_like(chosen_len), chosen_len
                )

            pred_lens = pred_lens[beam_idx] + chosen_len
            # Keep masks aligned with beams
            src_conf_high = src_conf_high[beam_idx]
            src_lens = src_lens[beam_idx]
...
def process_reward_beam_search(
    ...
    beam_scorer: BeamScorer,
    src_confidence_scores: Optional[torch.Tensor] = None,
    input_ids: torch.LongTensor = None,
